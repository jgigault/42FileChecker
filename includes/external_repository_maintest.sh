      ...
      then
        ...
        then
          ...
          then
            ...
            if [ "${TOTAL}" == "${SUCCESS}" ]
            then
              display_success  "All tests passed (${TOTAL} tests)"
            else
              display_error  "$(( ${TOTAL} - ${SUCCESS} )) failed test(s) out of ${TOTAL} tests"
            fi
          else
            display_error  "Compilation failed"
          fi
        else
          display_error  "An error occured while preparing main.c"
        fi
      else
        display_error  "An error occured while compiling your library"
      fi
    else
